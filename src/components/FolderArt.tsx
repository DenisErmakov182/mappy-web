...
export function FolderArt({
  ...
  return (
    <div className="relative" style={{ aspectRatio: `${BASE_W} / ${BASE_H}` }}>
      ...
      {/* Контактная тень (узел 2374:12427/2364:12389) — раньше рисовалась
          только у заполненной папки, но владелец 20.08.2026 добавил этот
          слой и в Property 1=Default (пустая, без фото) с теми же
          координатами — теперь безусловно, для любой папки, не только
          photos.length>0. Идёт ПЕРЕД фото и ПОД карманом — блюр (12.147px в
          макете) частично перекрывается карманом снизу, видимый ободок
          остаётся мягким затемнением над линией кармана. */}
      <span
